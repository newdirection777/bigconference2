...
#######################
	...
	@endpoints.method(StringMessage, SessionForms, path='getSessionsByTime',
			http_method='GET', name='getSessionsByTime')
	def getSessionsByTime(self, request):
		"""Get Sessions starting at a particular time."""
		
		q = Session.query()
		
		# convert input string to time object
		time = datetime.strptime(request.data[:5], "%H:%M").time()
	...
	def _getFeaturedSpeaker(speaker, websafeConferenceKey):
		...
		q = q.filter(Session.websafeConferenceKey==websafeConferenceKey)
